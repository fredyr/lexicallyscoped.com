...
Elm.BinClock = Elm.BinClock || {};
Elm.BinClock.make = function (_elm) {
   "use strict";
   _elm.BinClock = _elm.BinClock || {};
   if (_elm.BinClock.values)
   return _elm.BinClock.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var toBitNum = function (n) {
      return function () {
         var setp = function (x) {
            return _U.cmp(x,0) > 0;
         };
         var masks = A3($List.map2,
         $Bitwise.and,
         _L.fromArray([8,4,2,1]),
         A2($List.repeat,4,n));
   ...
   var decimalParts = function (n) {
      return _L.fromArray([n / 10 | 0
                          ,A2($Basics.rem,n,10)]);
   };
   var cellVal = function (digit) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("cell")]),
      _L.fromArray([$Html.text($Basics.toString(digit))]));
   };
   var legend = function (digits) {
      return function () {
         var cells = A2($List.map,
         cellVal,
         digits);
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("col legend")]),
         cells);
      }();
   };
   var cellCol = function (bit) {
      return function () {
         var color = bit ? "light" : "dark";
         return A2($Html.div,
   ...
   var column = function (_v0) {
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 var cells = A2($Basics._op["++"],
                 A2($List.map,cellCol,_v0._1),
                 _L.fromArray([cellVal(_v0._0)]));
                 return A2($Html.div,
                 _L.fromArray([$Html$Attributes.$class("col")]),
                 cells);
              }();}
         _U.badCase($moduleName,
   ...
   var columns = function (cs) {
      return function () {
         var cols = A2($List.map,
         column,
         cs);
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("colpair")]),
         cols);
      }();
   };
   var view = function (t) {
      return function () {
         ...
         var conv = function (x) {
            return $List.map(toBitNum)(decimalParts(x));
         };
         var cols = $List.map(columns)($List.map(conv)(_L.fromArray([$Date.hour(d)
                                                                    ,$Date.minute(d)
                                                                    ,$Date.second(d)])));
         return A2($Html.div,
         _L.fromArray([]),
         A2($Basics._op["++"],
         _L.fromArray([legend(_L.fromArray([8
                                           ,4
                                           ,2
                                           ,1]))]),
         cols));
